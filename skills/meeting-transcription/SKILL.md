...
| 項目 | Gemini 模式 | 本地模式 |
|---|---|---|
| 轉錄方式 | Google Gemini API | 本地 Whisper GPU 伺服器 |
| 會議記錄生成 | Gemini API | **Agent（Claude）** |
| 速度 | 快 | 慢（GPU 處理） |
| 費用 | Gemini 計費 | 免費（本地） |
| 步驟 | 一鍵完成 | 三步驟 |
...
**類型 1 — 商務會議** → 寫入 `xxx_會議記錄.md`（標題：`# xxx 會議記錄`）
  ...
  - `## 會議重點摘要`（3-8 條重點，直接從逐字稿萃取，忠實反映原文）
  - `## 決議事項清單`（table 格式：決議內容 / 負責人 / 期限）
    ⚠️ **負責人必須從逐字稿中出現的實際報告者（說話者）中選出，可以是多位，不可填「開發團隊」、「架構團隊」等泛稱**
  - `## 待辦追蹤事項`（table 格式：追蹤事項 / 負責人 / 期限）
    ⚠️ **負責人必須從逐字稿中出現的實際報告者（說話者）中選出，可以是多位，不可填泛稱**
  - `## 需要上報的關鍵資訊`
  - `## 與會人報告重點`（每位說話者一個小節，條列其在會議中報告的主要內容；若某說話者只有簡短回應可略過）
...
| 類型 | 輸出章節 |
|---|---|
| 1. 商務會議 | AI模型註記、會議重點摘要、決議事項清單（負責人從說話者選出）、待辦追蹤事項（負責人從說話者選出）、需要上報的關鍵資訊、與會人報告重點 |
